...
bool RouterInterface::SendQueryRequest(string apMacAddr)
{
    ...
    if(res != CURLE_OK)
    {
        cout<<"ERROR: curl_easy_perform failed " << dec <<  (int) res << endl;
    }
    else
    {
...
bool RouterInterface::UpdateIptables(string macAddr, string opCode) 
{
        ...
        if (string::npos != iptablesEntry.find(macAddr))
        {
            ...
            if(string::npos != actionPos)
            {
                ...
                if(prevAction != opCode)
                {
                    ...
                    replaceCmd << "iptables -C FORWARD -m mac --mac-source ";
    ...
    if( !bFound )
    {
        ...
        addNewEntry << "iptables -A FORWARD -m mac --mac-source " << macAddr << " -j "  << opCode  << endl;
    ...
    if (bNeedUpdate)
    {
        ...
        saveCmd << "iptables-save > " << IPTABLESCONFIGFILE ;
